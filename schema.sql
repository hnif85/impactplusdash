...
CREATE TABLE public.app_usage_logs (
  ...
  CONSTRAINT app_usage_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.app_users(id),
  CONSTRAINT app_usage_logs_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.companies(id)
);
...
CREATE TABLE public.baseline_umkm_profiles (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  app_user_id uuid,
  company_id uuid NOT NULL,
  ...
  promo_platforms ARRAY,
  social_media_impact text,
  content_method text,
  content_frequency text,
  content_difficulty smallint,
  ...
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT baseline_umkm_profiles_pkey PRIMARY KEY (id),
  CONSTRAINT baseline_umkm_profiles_app_user_id_fkey FOREIGN KEY (app_user_id) REFERENCES public.app_users(id),
  CONSTRAINT baseline_umkm_profiles_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.companies(id),
  CONSTRAINT baseline_umkm_profiles_source_response_id_fkey FOREIGN KEY (source_response_id) REFERENCES public.survey_responses(id)
);
...
CREATE TABLE public.dashboard_activity_logs (
  ...
  user_agent text,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT dashboard_activity_logs_pkey PRIMARY KEY (id),
  CONSTRAINT dashboard_activity_logs_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.dashboard_users(id)
);
CREATE TABLE public.dashboard_users (
  ...
  full_name character varying,
  avatar_url text,
  role character varying NOT NULL CHECK (role::text = ANY (ARRAY['super_admin'::character varying, 'company_admin'::character varying]::text[])),
  company_id uuid,
  is_active boolean DEFAULT true,
  ...
  CONSTRAINT dashboard_users_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.companies(id)
);
...
CREATE TABLE public.profile (
  ...
  CONSTRAINT profile_event_id_fkey FOREIGN KEY (event_id) REFERENCES public.training_events(id),
  CONSTRAINT profile_customer_guid_fkey FOREIGN KEY (customer_guid) REFERENCES public.cms_customers(guid)
);
...
CREATE TABLE public.survey_questions (
  ...
  question_type character varying NOT NULL CHECK (question_type::text = ANY (ARRAY['text'::character varying, 'multiple_choice'::character varying, 'checkbox'::character varying, 'rating'::character varying, 'nps'::character varying, 'yes_no'::character varying, 'dropdown'::character varying]::text[])),
...
CREATE TABLE public.surveys (
  ...
  description text,
  survey_type character varying DEFAULT 'general'::character varying CHECK (survey_type::text = ANY (ARRAY['general'::character varying, 'satisfaction'::character varying, 'feedback'::character varying, 'nps'::character varying]::text[])),
  is_active boolean DEFAULT true,
  start_date date,
  end_date date,
  created_by uuid,
  ...
  CONSTRAINT surveys_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.dashboard_users(id)
);
